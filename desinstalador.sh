...
echo -e "${GREEN}3. Eliminando todos los PV...${NC}"
kubectl delete -f pv/ --ignore-not-found
kubectl get pv --no-headers | awk '{print "kubectl delete pv", $1}' | bash
sleep 5

echo -e "${GREEN}4. Eliminando StorageClass 'nfs-storage-sc' y 'nfs-csi-jupyterhub-sc'...${NC}"
...
echo -e "${GREEN}5. Eliminando CRDs de Ray...${NC}"
