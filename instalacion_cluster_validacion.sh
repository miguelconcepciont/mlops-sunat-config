...
do
  ...
  bosh -d service-instance_6027e107-390c-4892-9d1a-c67d5a01a0ce ssh $server "sudo mkdir -p $folder_path && sudo chmod -R 777 $folder_path"
  bosh -d service-instance_6027e107-390c-4892-9d1a-c67d5a01a0ce ssh $server "sudo echo 'export https_proxy=https://192.168.56.85:3128' >> .bashrc"
  bosh -d service-instance_6027e107-390c-4892-9d1a-c67d5a01a0ce ssh $server ". .bashrc"
done
...
helm install postgresql ./postgresql-11.9.11.tgz --values config-postgresql.yaml
sleep 10
...
helm install postgresql-jupyterhub ./postgresql-11.9.11.tgz --values config-postgresql-jupyterhub.yaml
sleep 10
...
helm install minio ./minio-11.10.9.tgz --values config-minio.yaml
sleep 10
...
helm install mlflow ./mlflow-0.7.13.tgz --values config-mlflow.yaml
sleep 10
...
helm install kuberay-operator ./kuberay-operator-0.4.0.tgz
sleep 5
helm install raycluster ./ray-cluster-0.4.0.tgz --values config-ray.yaml
sleep 10
